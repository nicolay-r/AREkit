...
class BaseNeutralAnnotator(object):
    ...
    def __iter_all_doc_ids(self):
        assert(isinstance(self.__opin_ops, BaseExperiment))
        for data_type in DataType.iter_supported():
            for doc_id in self.__opin_ops.DocumentOperations.iter_news_indices(data_type):
                yield doc_id
    ...
    def filter_non_created_doc_ids(self, all_doc_ids, data_type):
        ...
        for doc_id in all_doc_ids:
            ...
            filepath = self.__opin_ops.create_neutral_opinion_collection_filepath(
                doc_id=doc_id,
                data_type=data_type)
    ...
    def iter_doc_ids_to_compare(self):
        ...
        for doc_id in self.__opin_ops.iter_doc_ids_to_compare(doc_ids_iter):
            yield doc_id
